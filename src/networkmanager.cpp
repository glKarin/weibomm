...
namespace id
{
	static const QStringList _SpecialUrl(QStringList() << "weibo" << "sina");
}
...
#if _DBG
		QList<QByteArray> headers = r.rawHeaderList();
		qDebug() << "----------" << r.url().toString();
		ID_CONST_FOREACH(QList<QByteArray>, headers)
		{
			qDebug() << *itor << ": " << r.rawHeader(*itor);
		}
		qDebug() << cookieJar()->cookiesForUrl(r.url());
		qDebug() << "---------------------------------";
#endif
...
void idNetworkAccessManager::Init()
{
    ...
    oHeaders.AddHeader("User-Agent", "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/57.0.2987.133 Safari/537.36");
    oHeaders.AddHeader("Referer", "https://m.weibo.cn/");
}
